...
export const ConverterTool: React.FC<ConverterToolProps> = () => {
  ...
  // Initialize PDF.js worker (reusing logic from SplitTool for safety)
  useEffect(() => {
    const initPdfWorker = () => {
  ...
  const handleFilesSelected = (files: File[]) => {
    ...
    if (firstFile.type === 'application/pdf') {
      setMode('pdf-to-x');
      setFile(firstFile);
      setTargetFormat('docx'); // Default to Word as per request
  ...
  const convertPdfToDocx = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Simple text extraction - consolidating lines
        let currentLine = "";
        let lastY = -1;

        for (const item of items as any[]) {
            // Very basic layout reconstruction based on Y coordinate
        ...
        if (i < pdf.numPages) {
            paragraphs.push(new Paragraph({
                children: [new TextRun({ text: "", break: 1 })] // Soft break, proper page break in docx needs PageBreak()
            }));
        }
      ...
      // Create Flattened Children array for single section doc (simplest for now)
  ...
  const convertImagesToDocx = async () => {
    ...
    try {
        ...
        for (const imgFile of imageFiles) {
            ...
            // Basic handling: fit to ~500px width
            paragraphs.push(new Paragraph({
                children: [
                    new ImageRun({
                        data: buffer,
                        transformation: {
                            width: 500,
                            height: 500 // Note: Keeping aspect ratio requires reading image dims, skipping for MVP stability
                        },
    ...
    <div className="max-w-2xl mx-auto space-y-6">
       ...
       <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center justify-between">
          ...
          <button onClick={handleReset} className="text-slate-400 hover:text-red-500 p-2">
             <AlertCircle size={20} className="rotate-45" /> {/* Using as Close icon substitute */}
          </button>
       </div>
